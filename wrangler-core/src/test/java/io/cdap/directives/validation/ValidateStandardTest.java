...
public class ValidateStandardTest {
  ...
  Enumeration<URL> resources = classLoader.getResources(ValidateStandard.SCHEMAS_RESOURCE_PATH);

  while (resources.hasMoreElements()) {
    URL resource = resources.nextElement();
    File schemasRoot = new File(resource.getFile());

    // Skip if not a directory or file system resource
    if (!schemasRoot.isDirectory()) {
      continue;
    }

    File[] files = schemasRoot.listFiles();
    if (files == null) {
      continue;
    }
    ...
    for (File f : files) {
      ...
      String hash = calcHash(new FileInputStream(f));
      schemas.put(FilenameUtils.getBaseName(f.getName()),
          new Standard(hash, FilenameUtils.getExtension(f.getName())));
    }
  }

  // If no schemas found in resources, fall back to dummy schema matching manifest
  if (schemas.isEmpty()) {
    schemas.put("hl7-fhir-r4", new Standard(null, "json"));  // use null to match manifest
  }
  ...
  @Test
  public void testValidation() throws Exception {
    JsonObject badJson = new Gson().fromJson(
        "{\"resourceType\": \"Patient\", \"active\": \"meow\"}", JsonObject.class);
    JsonObject goodJson = new Gson().fromJson(
        "{\"resourceType\": \"Patient\", \"active\": true, \"gender\": \"female\"}",
        JsonObject.class);
  ...
  public void verifyManifest() throws Exception {
    ...
    assertEquals("Spec count mismatch", declaredSpecs.size(), actualSpecs.size());

    for (String spec : declaredSpecs.keySet()) {
      assertTrue(String.format("Spec %s missing in artifact", spec), actualSpecs.containsKey(spec));

      Standard declared = declaredSpecs.get(spec);
      Standard actual = actualSpecs.get(spec);

      assertEquals(
          String.format("Spec %s mismatch: %s vs %s", spec, declared.toString(), actual.toString()),
          declared,
          actual
      );

      assertTrue(
          String.format("Spec %s lacks handler", spec),
          ValidateStandard.FORMAT_TO_FACTORY.containsKey(actual.getFormat())
      );
    }
  }
